...
public class AndroidTenjin : BaseTenjin {
	...
	public override void Init(string apiKey){
		SayKitDebug.Log ("Android Initializing " + apiKey);
		ApiKey = apiKey;
		initActivity();
	...
	public override void Init(string apiKey, string sharedSecret){
		SayKitDebug.Log ("Android Initializing with Shared Secret" + apiKey);
		ApiKey = apiKey;
	...
	public override void Transaction(string productId, string currencyCode, int quantity, double unitPrice, string transactionId, string receipt, string signature){
		...
		if(receipt != null && signature != null){
			...
			SayKitDebug.Log ("Android Transaction " + productId + ", " + currencyCode + ", " + quantity + ", " + unitPrice + ", " + receipt + ", " + signature);
			tenjinJava.Call ("transaction", receiptArgs);
		}
		else{
			object[] args = new object[]{productId, currencyCode, quantity, unitPrice};
			SayKitDebug.Log ("Android Transaction " + productId + ", " + currencyCode + ", " + quantity + ", " + unitPrice);
	...
	public override void Connect(){
		SayKitDebug.Log ("Android Connecting " + ApiKey);
	}

	public override void Connect(string deferredDeeplink){
		SayKitDebug.Log ("Android Connecting with deferredDeeplink " + deferredDeeplink);
	}

	public override void SendEvent (string eventName){
		SayKitDebug.Log ("Android Sending Event " + eventName);
	}

	public override void SendEvent (string eventName, string eventValue){
		SayKitDebug.Log ("Android Sending Event " + eventName + " : " + eventValue);
	}

	public override void Transaction(string productId, string currencyCode, int quantity, double unitPrice, string transactionId, string receipt, string signature){
		SayKitDebug.Log ("Android Transaction " + productId + ", " + currencyCode + ", " + quantity + ", " + unitPrice + ", " + transactionId + ", " + receipt + ", " + signature);
	}

	public override void GetDeeplink(Tenjin.DeferredDeeplinkDelegate deferredDeeplinkDelegate) {
		SayKitDebug.Log ("Sending AndroidTenjin::GetDeeplink");
	}

	public override void OptIn(){
		SayKitDebug.Log ("Sending AndroidTenjin::OptIn");
	}

	public override void OptOut(){
		SayKitDebug.Log ("Sending AndroidTenjin::OptOut");
	}

	public override void OptInParams(List<string> parameters){
		SayKitDebug.Log ("Sending AndroidTenjin::OptInParams");
	}

	public override void OptOutParams(List<string> parameters){
		SayKitDebug.Log ("Sending AndroidTenjin::OptOutParams");
	}
